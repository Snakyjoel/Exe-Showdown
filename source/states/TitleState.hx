...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl = new FlxSprite(-100, titleJSON.titley);
		...
		sonic = new FlxSprite().loadGraphic(Paths.image('sonic'));
		sonic.antialiasing = ClientPrefs.data.antialiasing;
		sonic.setGraphicSize(Std.int(sonic.width * 0.20));
		sonic.updateHitbox();
		sonic.x = (FlxG.width / 2) - (sonic.width / 2) + 400;
		sonic.screenCenter(Y);
		add(sonic);
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					case 'RIVER?=#J)R¡=Q"J¡=JF¡=#J¡R)FJA¡F)=(EHJ¡) boquita el mas grande':
						sound = FlxG.sound.play(Paths.sound('JingleRiver'));
					case 'SHADOWFE#H()"?)#(FH?"()FH#?"(FH)¡QEHF) gato femboy':
						FlxG.sound.play(Paths.sound('JingleShadow'));
					case 'BBPANZUF)(H"=)FH("?)RFH(="HF?E(WHQ?=)) snaky estuvo aqui':
